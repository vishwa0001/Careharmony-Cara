...
class CaraHealthBotDeployer:
    ...
    def ensure_identity_lex(
        ...
        if alias and self._identity_version_is_correct(bot_id, version):
            self.log(f"    Existing identity Live version {version} already matches V8 conversation-tested Cara identity gate")
        else:
            ...
            if not self._identity_version_is_correct(bot_id, version):
                raise DeploymentError(
                    "Published identity Lex version does not match the V6 Cara conversational identity gate"
                )
    ...
    def ensure_availability_lex(
        ...
        if alias and self._availability_version_is_correct(bot_id, version):
            self.log(f"    Existing availability Live version {version} already matches V2 safety-first availability behavior")
        else:
    ...
    def verify(self) -> dict[str, Any]:
        ...
        try:
            ...
            if first_routes.get("IdentityDenied") != "a0000000-0000-4000-8000-000000000001":
                raise DeploymentError("IdentityDenied is not routed to privacy-minimal availability handling")
            ...
            if first_routes.get("PatientUnavailable") != "a0000000-0000-4000-8000-000000000011":
                raise DeploymentError("PatientUnavailable is not routed directly to callback availability")
            if first_routes.get("ThirdPartyDetected") != "a0000000-0000-4000-8000-000000000001":
                raise DeploymentError("ThirdPartyDetected is not routed to the availability conversation")
            ...
            if second_routes.get("PatientUnavailable") != "a0000000-0000-4000-8000-000000000011":
                raise DeploymentError("PatientUnavailable attempt 2 is not routed directly to callback availability")
            if second_routes.get("ThirdPartyDetected") != "a0000000-0000-4000-8000-000000000001":
                raise DeploymentError("ThirdPartyDetected attempt 2 is not routed to availability")
            if second_routes.get("IdentityAmbiguous") != "a0000000-0000-4000-8000-000000000001":
                raise DeploymentError("IdentityAmbiguous attempt 2 is not routed to privacy-minimal availability")
            if second_routes.get("FallbackIntent") != "a0000000-0000-4000-8000-000000000001":
                raise DeploymentError("FallbackIntent attempt 2 is not routed to privacy-minimal availability")
            if identity_2.get("Transitions", {}).get("NextAction") != "a0000000-0000-4000-8000-000000000001":
                raise DeploymentError("Unresolved identity attempt 2 does not default to privacy-minimal availability")
            ...
            if compare_values != {"$.Lex.SessionAttributes.Tool", "$.Lex.SessionAttributes.endReason"}:
                raise DeploymentError("Published Cara outcome routers are missing or malformed")
