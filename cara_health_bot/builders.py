...
def availability_callback_time_slot_request(
    ...
    return {
        ...
        "valueElicitationSetting": {
            ...
            "promptSpecification": {
                "messageGroups": [{
                    "message": {"plainTextMessage": {"value": "What time would be better to reach them?"}}
                }],
                "maxRetries": 1,
                "allowInterrupt": True,
            },
...
def cara_return_to_control_tools() -> list[dict[str, Any]]:
    ...
    return [
        ...
        },
        {
            ...
            "instruction": {
                "instruction": (
                    "Use when the customer says they are busy, asks to be called later, or gives a preferred "
                    "callback time and there is no safety concern. Safety always overrides callback. Never invent "
                    "a date or time. Before invoking, verbally acknowledge any specific time the customer actually provided."
                )
            },
            "inputSchema": {
                "type": "object",
        ...
        },
        {
            ...
            "instruction": {
                "instruction": (
                    ...
                    "inappropriate. Safety has the highest priority over every other intent. For urgent physical/medical "
