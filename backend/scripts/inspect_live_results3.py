...
res = table.scan(FilterExpression=Attr("batchId").eq("live-test-1788186905"))
...
for i, item in enumerate(items, 1):
    ...
    print(f"callStartDateTime: {item.get('callStartDateTime')}")
    print(f"callEndDateTime: {item.get('callEndDateTime')}")
    print(f"completedAt: {item.get('completedAt')}")
